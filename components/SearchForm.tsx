...
export const formSchema = z.object({
  ...
  adults: z
    .string()
    .min(1, { message: "Please at least select 1 Adult" })
    .max(12, { message: "Max. 12 adults occupancy" }),
  children: z.string().min(0).max(12, { message: "Max 12 children occupancy" }),
  room: z.string().min(1, { message: "Please select at least one room" }),
});
...
function SearchForm() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const checkoutMonthDay = values.dates.from.getDate().toString();
    const checkoutMonth = (values.dates.from.getMonth() + 1).toString();
    const checkoutYear = values.dates.from.getFullYear().toString();
    ...
    url.searchParams.set("no_rooms", values.room);
  ...
  return (
    <Form {...form}>
      ...
      >
        <div className="w-full grid lg:max-w-sm items-center gap-1.5">
          <FormField
        ...
        <div className="flex w-full items-center space-x-2">
          ...
          <div className="mt-auto">
            <button className="text-base bg-blue-600 font-semibold text-white rounded-lg px-2 lg:px-4 py-2">
              Search
            </button>
          </div>
        </div>
